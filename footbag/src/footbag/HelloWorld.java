...
//					float[] col = {(float)(Math.random()),(float)(Math.random()),(float)(Math.random())};
					float[] col = dotU.getColor(bg.getDot(i, j));
					drawSquare( i*pixWidth, j*pixWidth, pixWidth, pixWidth, col);
					col = dotU.getColor(map.getDot(i, j));
	...
	private void input() {
		...
		if(glfwGetKey(window, GLFW_KEY_SPACE)==1){
			map.putDot(player.getX()+player.getWidth(), player.getY()+player.getHeight()-1, new Dot((byte)1, (float)(Math.random()*0.5 + 0.5)));
		}
	}
...
//		glEnd();
	...
	private boolean checkColision(Entity e, Map m){
		boolean temp = false;
		
		for(int i = 0; i < player.getMap().getWidth(); i++){
			
			for(int j = 0; j < player.getMap().getHeight(); j++){
				if(player.getMap().getDot(i, j).getType() != (byte)0 && map.getDot(i+player.getX(), j+player.getY()).getType() != (byte)0){
					temp = true;
				}
			}
